...
@section('content')
    <div class="space-y-4 sm:space-y-6 pt-3 md:pt-0">
        ...
        <div class="mb-4 sm:mb-6">
            ...
            <div class="flex items-center gap-3 mb-4 md:hidden" style="padding-top: 2.5rem;">
                ...
                <button id="page-mobile-menu-button"
                    class="flex-shrink-0 p-2 rounded-lg bg-white border border-gray-300 shadow-md hover:bg-gray-50 transition-colors dark:text-white dark:bg-gray-700 dark:hover:bg-gray-600"
                    ...
                    <svg id="page-close-icon" class="h-5 w-5 hidden" fill="none" viewBox="0 0 24 24" stroke-width="2"
                        stroke="currentColor" class="text-gray-900 dark:text-white">
            ...
            <div class="mb-4 md:hidden">
                <a href="{{ route('admin.services.create') ?? route('services.create') ?? '#' }}"
                    class="inline-flex items-center justify-center w-full px-4 py-2.5 border border-transparent rounded-lg shadow-sm text-sm font-medium transition-colors dark:text-white dark:bg-green-600 dark:hover:bg-green-700">
            ...
            <div class="hidden md:flex md:items-center md:justify-between">
                ...
                <div class="mt-3 sm:mt-4 md:mt-0 md:ml-4">
                    <a href="{{ route('admin.services.create') ?? route('services.create') ?? '#' }}"
                        class="inline-flex items-center justify-center w-full sm:w-auto px-3 sm:px-4 py-2 border border-transparent rounded-lg shadow-sm text-xs sm:text-sm font-medium transition-colors dark:text-white dark:bg-green-600 dark:hover:bg-green-700"
                        style="border: 1px solid #e5e7eb !important;">
                        <svg class="w-4 h-4 sm:w-5 sm:h-5 mr-1.5 sm:mr-2" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                            stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
                        </svg>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 md:p-6 mb-6">
            ...
            <form method="GET" action="{{ route('admin.services.index') ?? route('services.index') ?? '#' }}"
                class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <div class="flex flex-col space-y-2">
                    <label for="status" class="text-sm font-medium text-gray-700 dark:text-white">Estado</label>
                    <select name="status" id="status"
                        class="border border-gray-300 rounded-lg px-4 py-3.5 text-base focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500 w-full dark:text-white dark:bg-gray-700 dark:border-gray-600">
                        <option value="">Todos los estados</option>
                        <option value="pendiente" {{ request('status') === 'pendiente' ? 'selected' : '' }}>Pendiente</option>
                        <option value="en_progreso" {{ request('status') === 'en_progreso' ? 'selected' : '' }}>En Progreso
                        </option>
                ...
                <div class="flex flex-col space-y-2">
                    ...
                    <select name="type" id="type"
                        ...
                        <option value="desratizacion" {{ request('type') === 'desratizacion' ? 'selected' : '' }}>
                            Desratización</option>
                        <option value="desinsectacion" {{ request('type') === 'desinsectacion' ? 'selected' : '' }}>
                            Desinsectación</option>
                        <option value="sanitizacion" {{ request('type') === 'sanitizacion' ? 'selected' : '' }}>Sanitización
                        </option>
                    </select>
                </div>
                <div class="flex flex-col space-y-2">
                ...
                <div class="flex flex-col space-y-2">
                    ...
                    <div class="flex items-center gap-3 h-full">
                        <button type="submit" id="filter-submit-btn"
                            class="flex-1 px-6 py-3.5 bg-gray-700 hover:bg-gray-800 text-white font-medium rounded-lg shadow-sm transition-colors dark:bg-green-600 dark:hover:bg-green-700">
                            Filtrar
                        </button>
                        <a href="{{ route('admin.services.index') }}"
                            class="flex-1 px-6 py-3.5 bg-gray-200 hover:bg-gray-300 text-gray-700 font-medium rounded-lg shadow-sm transition-colors text-center no-underline">
                            Limpiar
                        </a>
                    </div>
                </div>
        ...
        <!-- Services List -->
        <div
            class="bg-white dark:bg-gray-800 border dark:border-gray-700 rounded-lg overflow-hidden border border-gray-200 dark:border-gray-700">
            <div class="overflow-x-auto">
                @forelse($services as $service)
                    <div class="p-4 sm:p-6 border-b border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700"
                        ...
                        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                            <div class="flex-1">
                                <div class="mb-2">
                                    ...
                                    @if($service->address)
                                        <p
                                            class="text-sm text-gray-600 dark:text-white hover:text-gray-900 dark:hover:text-gray-400">
                                            {{ $service->address }}</p>
                                    @endif
                                </div>
                            ...
                            <div class="flex items-center gap-2">
                                <a href="{{ route('admin.services.show', $service) ?? route('services.show', $service) ?? '#' }}"
            ...
            @if($services->hasPages())
                <div class="bg-white px-2 sm:px-6 py-3 border-t border-gray-200"
                    style="border-top: 1px solid #e5e7eb !important;">
                    ...
                    <div class="flex items-center justify-center gap-1 overflow-x-auto w-full">
                        @if($services->onFirstPage())
                            <span
                                class="px-2 py-1.5 text-xs sm:text-sm font-medium text-gray-400 cursor-not-allowed whitespace-nowrap"
                                style="color: #9CA3AF !important;">« Anterior</span>
                        @else
                            <a href="{{ $services->previousPageUrl() }}"
                                class="px-2 py-1.5 text-xs sm:text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 whitespace-nowrap"
                                style="color: #9CA3AF !important;">« Anterior</a>
                        @endif
                        ...
                        @if($startPage > 1)
                            <a href="{{ $services->url(1) }}"
                                class="px-2 py-1.5 text-xs sm:text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 whitespace-nowrap"
                                style="color: #9CA3AF !important;">1</a>
                            @if($startPage > 2)
                        ...
                        @foreach($services->getUrlRange($startPage, $endPage) as $page => $url)
                            @if($page == $services->currentPage())
                                <span
                                    class="px-2.5 py-1.5 text-xs sm:text-sm font-medium text-white bg-green-600 border border-green-600 rounded-md whitespace-nowrap">{{ $page }}</span>
                            @else
                                <a href="{{ $url }}"
                                    class="px-2.5 py-1.5 text-xs sm:text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 whitespace-nowrap"
                                    style="color: #9CA3AF !important;">{{ $page }}</a>
                            @endif
                        @endforeach
                        ...
                        @if($endPage < $lastPage)
                            ...
                            <a href="{{ $services->url($lastPage) }}"
                                class="px-2 py-1.5 text-xs sm:text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 whitespace-nowrap"
                                style="color: #9CA3AF !important;">{{ $lastPage }}</a>
                        @endif

                        @if($services->hasMorePages())
                            <a href="{{ $services->nextPageUrl() }}"
                                class="px-2 py-1.5 text-xs sm:text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 whitespace-nowrap"
                                style="color: #9CA3AF !important;">Siguiente »</a>
                        @else
                            <span
                                class="px-2 py-1.5 text-xs sm:text-sm font-medium text-gray-400 cursor-not-allowed whitespace-nowrap"
                                style="color: #9CA3AF !important;">Siguiente »</span>
                        @endif
                    </div>
