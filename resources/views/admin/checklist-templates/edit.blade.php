...
@section('title', 'Editar Template de Checklist - Pest Controller SAT')
@section('page-title', 'Editar Template de Checklist')

@section('content')
<div class="max-w-4xl mx-auto">
    <form action="{{ route('admin.checklist-templates.update', $checklistTemplate) }}" method="POST" x-data="checklistForm()" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex justify-between items-center mb-6">
                <h2 class="text-xl font-semibold text-gray-900">Editar Template: {{ $checklistTemplate->name }}</h2>
                <a href="{{ route('admin.checklist-templates.show', $checklistTemplate) }}" 
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $checklistTemplate->name) }}" required
                ...
                <div>
                    ...
                    <select name="service_type_id" required
                        ...
                        <option value="{{ $serviceType->id }}" {{ old('service_type_id', $checklistTemplate->service_type_id) == $serviceType->id ? 'selected' : '' }}>
                            {{ $serviceType->name }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <div class="flex items-center">
                        <input type="checkbox" name="is_active" value="1" 
                               {{ old('is_active', $checklistTemplate->is_active) ? 'checked' : '' }}
                               class="rounded border-gray-300 text-green-600 focus:ring-green-500">
                        <label class="ml-2 text-sm text-gray-700">Template activo</label>
                    </div>
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" rows="3"
                              ...
                              placeholder="Descripción opcional del template">{{ old('description', $checklistTemplate->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
        ...
        <div class="flex justify-end space-x-4">
            ...
            <button type="submit" 
                ...
                <i class="fas fa-save mr-2"></i>Actualizar Template
            </button>
        </div>
    </form>
</div>
...
function checklistForm() {
    return {
        items: {!! json_encode($checklistTemplate->items->map(function($item) {
            return [
                'title' => $item->title,
