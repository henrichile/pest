...
<head>
    ...
    <style>
        ...
        .header {
            ...
            position: relative;
            min-height: 80px;
            /* Asegurar espacio para logo y QR */
        }
...
<body>
    <div class="header">
        <div class="logo">
            @php
                // Buscar el logo en el mismo orden que la configuración del sistema
                $logoPath = null;

                // Prioridad: logo.png > logo.jpg > logo.svg
                if (file_exists(public_path('logo.png'))) {
                    ...
                    // SVG no es compatible con DomPDF, intentar con versión PNG si existe
                    $logoPath = null;
                }
                ...
                if ($logoPath && file_exists($logoPath)) {
                    ...
                    $logoBase64 = 'data:image/' . ($type === 'jpg' ? 'jpeg' : $type) . ';base64,' . base64_encode($data);
                }
            @endphp
            @if($logoBase64)
    ...
    <div class="service-info">
        ...
        @if($service->latitude && $service->longitude)
            <div class="geolocation-info">
                ...
                <div class="info-row">
                    ...
                    <span
                        class="info-value">https://maps.google.com/?q={{ $service->latitude }},{{ $service->longitude }}</span>
                </div>
                ...
                @if(isset($mapBase64) && $mapBase64)
                    <div
                        style="margin-top: 10px; margin-bottom: 5px; border: 1px solid #e5e7eb; border-radius: 4px; overflow: hidden;">
                        <img src="{{ $mapBase64 }}" style="width: 100%; max-height: 200px; object-fit: cover; display: block;"
        ...
        <div class="info-row">
            ...
            <span
                class="info-value">{{ $service->serviceType->name ?? ucfirst(str_replace('-', ' ', $service->service_type)) }}</span>
        </div>
        <div class="info-row">
        ...
        <div class="info-row">
            ...
            <span
                class="info-value">{{ $service->scheduled_date ? $service->scheduled_date->format("d/m/Y H:i") : "N/A" }}</span>
        </div>
        <div class="info-row">
            <span class="info-label">Fecha de Finalización:</span>
            <span
                class="info-value">{{ $service->checklist_completed_at ? $service->checklist_completed_at->format("d/m/Y H:i") : "N/A" }}</span>
        </div>
        <div class="info-row">
        ...
        @if(isset($checklistData['monitoreo_completo']))
            <div class="section">
                ...
                @if(isset($checklistData['monitoreo_completo']['monitoring_date']))
                    <div class="checklist-item">
                        <strong>Fecha de Monitoreo:</strong>
                        {{ \Carbon\Carbon::parse($checklistData['monitoreo_completo']['monitoring_date'])->format('d/m/Y') }}
                    </div>
                @endif

                @if(isset($checklistData['monitoreo_completo']['total_bait_stations']))
                    <div class="checklist-item">
                        <strong>Total de Cebaderas Instaladas:</strong>
                        {{ $checklistData['monitoreo_completo']['total_bait_stations'] }}
                    </div>
                @endif
                                    ...
                                    <div class="bait-station-detail">
                                        ...
                                        <div class="photo-grid">
                                            @foreach($station['photos'] as $photo)
                                                ...
                                                @if($imageSrc)
                                                    <div class="photo-item">
                                                        <img src="{{ $imageSrc }}" alt="Foto de cebadera {{ $station['code'] ?? ($index + 1) }}"
                                                            class="bait-station-photo">
                                                    </div>
                                                @endif
                                            @endforeach
                                        </div>
                        ...
                        @foreach($checklistData['monitoreo_completo']['traps'] as $index => $trap)
                            <div class="bait-station-card">
                                ...
                                @if(isset($trap['product_type']))
                                    <div class="bait-station-detail"><strong>Producto/Material:</strong>
                                        {{ ucfirst($trap['product_type']) }}</div>
                                @endif
                                    ...
                                    <div class="bait-station-detail">
                                        ...
                                        <div class="photo-grid">
                                            @foreach($trap['photos'] as $photo)
                                                ...
                                                @if($imageSrc)
                                                    <div class="photo-item">
                                                        <img src="{{ $imageSrc }}" alt="Foto de trampa {{ $trap['code'] ?? ($index + 1) }}"
                                                            class="bait-station-photo">
                                                    </div>
                                                @endif
                                            @endforeach
        ...
        @php
            ...
            foreach ($baitStations as $station) {
                ...
                if (isset($station['observations']) && is_array($station['observations'])) {
                    ...
                    if (
                        in_array('bloqueada', $station['observations']) ||
        ...
        @if($totalMonitoreadas > 0)
            <div class="section">
                ...
                <table style="width: 100%; border-collapse: collapse; margin-bottom: 15px;">
                    <tr>
                        <td
                            style="width: 33.33%; padding: 10px; background: #f9fafb; border-radius: 5px; border: 1px solid #e5e7eb; text-align: center; vertical-align: top;">
                            <div
                                style="font-size: 9px; color: #6b7280; margin-bottom: 5px; font-weight: 600; text-transform: uppercase;">
                                CEBADERAS MONITOREADAS</div>
                            <div style="font-size: 20px; font-weight: bold; color: #111827;">{{ $totalMonitoreadas }}</div>
                        </td>
                        <td style="width: 10px;"></td>
                        <td
                            style="width: 33.33%; padding: 10px; background: #f0fdf4; border-radius: 5px; border: 1px solid #22c55e; text-align: center; vertical-align: top;">
                            <div
                                style="font-size: 9px; color: #6b7280; margin-bottom: 5px; font-weight: 600; text-transform: uppercase;">
                                CEBADERAS ACTIVAS</div>
                            <div style="font-size: 20px; font-weight: bold; color: #22c55e;">{{ $totalActivas }}</div>
                        </td>
                        <td style="width: 10px;"></td>
                        <td
                            style="width: 33.33%; padding: 10px; background: #fef3c7; border-radius: 5px; border: 1px solid #f59e0b; text-align: center; vertical-align: top;">
                            <div
                                style="font-size: 9px; color: #6b7280; margin-bottom: 5px; font-weight: 600; text-transform: uppercase;">
                                CON PROBLEMAS</div>
                ...
                {{-- Métricas Clave --}}
                <table
                    style="width: 100%; border-collapse: collapse; margin-bottom: 15px; background: #f9fafb; border-radius: 5px; border: 1px solid #e5e7eb;">
                    <tr>
                        ...
                        <td style="width: 33.33%; padding: 12px; text-align: center; vertical-align: top;">
                            ...
                            <div
                                style="font-size: 18px; font-weight: bold; color: {{ $consumoPromedio > 30 ? '#ef4444' : ($consumoPromedio > 10 ? '#f59e0b' : '#22c55e') }};">
                        ...
                        <td style="width: 33.33%; padding: 12px; text-align: center; vertical-align: top;">
                            ...
                            <div>
                                <span
                                    style="display: inline-block; padding: 4px 12px; border-radius: 12px; font-size: 10px; font-weight: bold; background: {{ $nivelColor }}; color: white; text-transform: uppercase;">
                                    {{ $nivelActual }}
                                </span>
                            </div>
                        </td>
    ...
    @if($service->service_type === 'desratizacion')
        <div class="section" style="page-break-before: auto; margin-top: 30px;">
            <div class="section-title"
                style="font-size: 18px; color: #1a472a; border-bottom: 2px solid #1a472a; padding-bottom: 10px; margin-bottom: 20px;">
                                ...
                                @if(isset($observation['photo']))
                                    ...
                                    @if($imageSrc)
                                        <div style="margin-top: 8px;">
                                            <img src="{{ $imageSrc }}" alt="Foto de observación"
        ...
        <div class="section" style="page-break-before: auto; margin-top: 30px;">
            ...
            @if(isset($checklistData['products']))
                <div class="subsection" style="margin-bottom: 20px;">
                    ...
                    <div class="checklist-item">
                        <strong>Producto aplicado:</strong>
                        {{ $checklistData['products']['applied_product'] ?? 'No especificado' }}<br>
                        <strong>Dosis aplicada:</strong>
                        {{ $checklistData['products']['dosis_aplicada'] ?? ($checklistData['products']['dosis'] ?? 'No especificado') }}
                        cc<br>
                        <strong>Agua aplicada:</strong>
                        {{ $checklistData['products']['agua_aplicada'] ?? ($checklistData['products']['agua'] ?? 'No especificado') }}
                        Lts
                    </div>
                </div>
            @endif
    ...
    <div class="signature-section">
        ...
        <div style="margin-top: 20px; text-align: center;">
            <div class="signature-label">Fecha de Finalización:
                {{ $service->checklist_completed_at ? $service->checklist_completed_at->format("d/m/Y H:i") : date("d/m/Y H:i") }}
            </div>
        </div>
    </div>
