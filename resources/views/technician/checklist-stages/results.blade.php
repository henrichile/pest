    ...
    <form method="POST" action="{{ $submitRoute }}" data-stage="results">
        ...
        @if($service->service_type === 'desratizacion')
            <div class="checkbox-group">
                ...
                @foreach($options as $number => $option)
                    <div class="checkbox-item">
                        <input type="checkbox" name="observed_results[]" value="{{ $option }}" {{ in_array($option, $observedResults) ? "checked" : "" }} id="result_{{ $number }}">
            ...
            <div class="form-group">
                ...
                <input type="number" name="total_installed_points"
                    value="{{ $service->checklist_data["results"]["total_installed_points"] ?? "" }}" min="0"
                    placeholder="Cantidad de puntos instalados">
            </div>

            <div class="form-group">
                <label>ACTIVIDAD DE CONSUMO TOTAL</label>
                <input type="number" name="total_consumption_activity"
                    value="{{ $service->checklist_data["results"]["total_consumption_activity"] ?? "" }}" step="0.01" min="0"
                    placeholder="Actividad de consumo">
            </div>
        @endif
        @if($service->service_type === 'desinsectacion')
            ...
            <div class="form-group">
                ...
                <input type="number" name="uv_lamps" value="{{ $service->checklist_data["results"]["uv_lamps"] ?? "" }}" min="0"
                    placeholder="Número de lámparas ultravioletas">
            </div>
            ...
            <div class="form-group">
                <label>TUV</label>
                <input type="number" name="tuv" value="{{ $service->checklist_data["results"]["tuv"] ?? "" }}" min="0"
                    placeholder="0TUV">
            </div>
            ...
            <div class="form-group">
                ...
                <input type="number" name="devices_installed"
                    value="{{ $service->checklist_data["results"]["devices_installed"] ?? "" }}" min="0"
                    placeholder="0Dispositivos instalados">
            </div>

            <div class="form-group">
                <label>DISPOSITIVOS EXISTENTES</label>
                <input type="number" name="devices_existing"
                    value="{{ $service->checklist_data["results"]["devices_existing"] ?? "" }}" min="0"
                    placeholder="0Dispositivos existentes">
            </div>

            <div class="form-group">
                <label>DISPOSITIVOS REPUESTOS</label>
                <input type="number" name="devices_replaced"
                    value="{{ $service->checklist_data["results"]["devices_replaced"] ?? "" }}" min="0"
                    placeholder="0Dispositivos repuestos">
            </div>
        @endif
