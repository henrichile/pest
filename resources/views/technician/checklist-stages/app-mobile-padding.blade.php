...
<body class="h-full" id="body-element" style="overflow-x: hidden; background: #f9fafb;">
    <div id="app" class="flex h-full flex-row">
        <!-- Mobile Overlay -->
        <div id="mobile-overlay" class="md:hidden fixed inset-0 bg-gray-900 bg-opacity-50 z-40 hidden"
            style="z-index: 9998;"></div>

        <!-- Sidebar -->
        <aside id="sidebar"
            class="fixed md:static flex-shrink-0 w-80 md:w-72 flex flex-col bg-white border-r border-gray-200 transition-transform duration-300 ease-in-out z-50 h-full"
            ...
            <div class="flex grow flex-col gap-y-3 overflow-y-auto px-6 md:px-5 pb-4">
                <div class="flex h-16 shrink-0 items-center justify-center pt-6 pb-4">
                    <a href="{{ auth()->user()->hasRole('super-admin') ? route('admin.dashboard') : route('technician.dashboard') }}"
                        ...
                        <img src="https://pestcontroller.cl/wp-content/uploads/2022/07/pestcontroller-logo.png"
                            alt="PestController Logo"
                            class="h-14 w-auto object-contain max-w-full hover:opacity-80 transition-opacity cursor-pointer">
                    </a>
                </div>
                ...
                <div class="mb-3">
                    <div id="dark-mode-container"
                        class="w-full flex items-center justify-between gap-x-3 rounded-lg py-2.5 px-3 text-xs transition-colors duration-200">
                        <div class="flex items-center gap-x-2">
                            <svg id="dark-mode-icon" class="h-4 w-4 transition-colors duration-200" fill="none"
                                viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
                                class="text-gray-600 dark:text-white">
                                <path stroke-linecap="round" stroke-linejoin="round"
                                    d="M12 3v2.25m6.364.386l-1.591 1.591M21 12h-2.25m-.386 6.364l-1.591-1.591M12 18.75V21m-4.773-4.227l-1.591 1.591M5.25 12H3m4.227-4.773L5.636 5.636M15.75 12a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0z" />
                            </svg>
                            <span id="dark-mode-text" class="text-xs font-medium transition-colors duration-200"
                                style="font-size: 11px; color: #111827;">Modo Claro</span>
                        </div>
                        <button id="dark-mode-toggle" type="button"
                            class="relative inline-flex h-7 w-12 items-center rounded-full transition-all duration-300 ease-in-out focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2"
                            style="background-color: #d1d5db;" role="switch" aria-checked="false">
                            <span id="dark-mode-switch-thumb"
                                class="inline-block h-6 w-6 transform rounded-full bg-white shadow-md transition-all duration-300 ease-in-out"
                                style="translate: 0.125rem 0;"></span>
                        </button>
                    </div>
                </div>
                    ...
                    @if(auth()->user()->hasRole('super-admin'))
                        @if(session('view_as_technician'))
                            <form action="{{ route('admin.stop-viewing-as-technician') }}" method="POST" class="mb-2.5">
                                @csrf
                                <button type="submit"
                                    class="w-full flex items-center gap-x-2 rounded-lg py-2 px-2.5 text-xs hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                        ...
                        @if($isViewingAsTechnician)
                            ...
                            <li>
                                ...
                                <ul role="list" class="-mx-2 space-y-2">
                                    <li>
                                        <a href="{{ route('technician-view.dashboard') }}"
                                            class="group flex items-center gap-x-4 md:gap-x-6 rounded-lg px-4 py-3.5 md:py-3 text-sm leading-5 font-medium {{ request()->routeIs('technician-view.dashboard') ? 'bg-green-500 text-white' : 'text-gray-900 dark:text-white hover:bg-gray-800 hover:text-white dark:hover:bg-gray-800' }}"
                                            style="{{ !request()->routeIs('technician-view.dashboard') ? 'color: #111827 !important;' : '' }}">
                                            <svg class="h-6 w-6 md:h-5 md:w-5 shrink-0 {{ request()->routeIs('technician-view.dashboard') ? 'text-white' : 'text-gray-900 dark:text-gray-400 group-hover:text-white' }}"
                                                style="margin-right: 12px !important; {{ !request()->routeIs('technician-view.dashboard') ? 'color: #111827 !important;' : '' }}"
                                                fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round"
                                                    d="M3.75 6A2.25 2.25 0 016 3.75h2.25A2.25 2.25 0 0110.5 6v2.25a2.25 2.25 0 01-2.25 2.25H6a2.25 2.25 0 01-2.25-2.25V6zM3.75 15.75A2.25 2.25 0 016 13.5h2.25a2.25 2.25 0 012.25 2.25V18a2.25 2.25 0 01-2.25 2.25H6A2.25 2.25 0 013.75 18v-2.25zM13.5 6a2.25 2.25 0 012.25-2.25H18A2.25 2.25 0 0120.25 6v2.25A2.25 2.25 0 0118 10.5h-2.25a2.25 2.25 0 01-2.25-2.25V6zM13.5 15.75a2.25 2.25 0 012.25-2.25H18a2.25 2.25 0 012.25 2.25V18A2.25 2.25 0 0118 20.25h-2.25A2.25 2.25 0 0113.5 18v-2.25z" />
                                            </svg>
                                            <span
                                                class="{{ request()->routeIs('technician-view.dashboard') ? 'text-white' : 'text-gray-900 dark:text-white group-hover:text-white' }}"
                                                style="font-size: 15px; font-weight: 500; {{ !request()->routeIs('technician-view.dashboard') ? 'color: #111827 !important;' : '' }}">Dashboard</span>
                                        </a>
                                    </li>
                                    <li>
                                        <a href="{{ route('technician-view.services') }}"
                                            class="group flex items-center gap-x-4 md:gap-x-6 rounded-lg px-4 py-3.5 md:py-3 text-sm leading-5 font-medium {{ request()->routeIs('technician-view.services') || request()->routeIs('technician-view.service.*') ? 'bg-green-500 text-white' : 'text-gray-900 dark:text-white hover:bg-gray-800 hover:text-white dark:hover:bg-gray-800' }}"
                                            style="{{ !(request()->routeIs('technician-view.services') || request()->routeIs('technician-view.service.*')) ? 'color: #111827 !important;' : '' }}">
                                            <svg class="h-6 w-6 md:h-5 md:w-5 shrink-0 {{ request()->routeIs('technician-view.services') || request()->routeIs('technician-view.service.*') ? 'text-white' : 'text-gray-900 dark:text-gray-400 group-hover:text-white' }}"
                                                style="margin-right: 12px !important; {{ !(request()->routeIs('technician-view.services') || request()->routeIs('technician-view.service.*')) ? 'color: #111827 !important;' : '' }}"
                                                fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round"
                                                    d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5a2.25 2.25 0 002.25-2.25m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5a2.25 2.25 0 012.25 2.25v7.5" />
                                            </svg>
                                            <span
                                                class="{{ request()->routeIs('technician-view.services') || request()->routeIs('technician-view.service.*') ? 'text-white' : 'text-gray-900 dark:text-white group-hover:text-white' }}"
                                                style="font-size: 15px; font-weight: 500; {{ !(request()->routeIs('technician-view.services') || request()->routeIs('technician-view.service.*')) ? 'color: #111827 !important;' : '' }}">Mis
                                                Servicios</span>
                                        </a>
                                    </li>
                                    <li>
                                        <a href="{{ route('technician-view.profile') }}"
                                            class="group flex items-center gap-x-4 md:gap-x-6 rounded-lg px-4 py-3.5 md:py-3 text-sm leading-5 font-medium {{ request()->routeIs('technician-view.profile') ? 'bg-green-500 text-white' : 'text-gray-900 dark:text-white hover:bg-gray-800 hover:text-white dark:hover:bg-gray-800' }}"
                                            style="{{ !request()->routeIs('technician-view.profile') ? 'color: #111827 !important;' : '' }}">
                                            <svg class="h-6 w-6 md:h-5 md:w-5 shrink-0 {{ request()->routeIs('technician-view.profile') ? 'text-white' : 'text-gray-900 dark:text-gray-400 group-hover:text-white' }}"
                                                style="margin-right: 12px !important; {{ !request()->routeIs('technician-view.profile') ? 'color: #111827 !important;' : '' }}"
                                                fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round"
                                                    d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z" />
                                            </svg>
                                            <span
                                                class="{{ request()->routeIs('technician-view.profile') ? 'text-white' : 'text-gray-900 dark:text-white group-hover:text-white' }}"
                                                style="font-size: 15px; font-weight: 500; {{ !request()->routeIs('technician-view.profile') ? 'color: #111827 !important;' : '' }}">Mi
                                                Perfil</span>
                                        </a>
                                    </li>
                                </ul>
                        ...
                        @else
                            ...
                            <li>
                                ...
                                <ul role="list" class="-mx-2 space-y-1.5">
                                    <li>
                                        <a href="{{ auth()->user()->hasRole('super-admin') ? route('admin.dashboard') : route('technician.dashboard') }}"
                                            class="group flex items-center gap-x-6 rounded-md px-3 py-3 text-sm leading-5 font-medium {{ request()->routeIs('dashboard') || request()->routeIs('admin.dashboard') || request()->routeIs('technician.dashboard') ? 'bg-green-500 text-white' : 'text-gray-900 dark:text-white hover:bg-gray-800 hover:text-white dark:hover:bg-gray-800' }}"
                                            style="{{ !(request()->routeIs('dashboard') || request()->routeIs('admin.dashboard') || request()->routeIs('technician.dashboard')) ? 'color: #111827 !important;' : '' }}">
                                            <svg class="h-5 w-5 shrink-0 {{ request()->routeIs('dashboard') || request()->routeIs('admin.dashboard') || request()->routeIs('technician.dashboard') ? 'text-white' : 'text-gray-900 dark:text-gray-400 group-hover:text-white' }}"
                                                style="margin-right: 20px !important; {{ !(request()->routeIs('dashboard') || request()->routeIs('admin.dashboard') || request()->routeIs('technician.dashboard')) ? 'color: #111827 !important;' : '' }}"
                                                fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round"
                                                    d="M3.75 6A2.25 2.25 0 016 3.75h2.25A2.25 2.25 0 0110.5 6v2.25a2.25 2.25 0 01-2.25 2.25H6a2.25 2.25 0 01-2.25-2.25V6zM3.75 15.75A2.25 2.25 0 016 13.5h2.25a2.25 2.25 0 012.25 2.25V18a2.25 2.25 0 01-2.25 2.25H6A2.25 2.25 0 013.75 18v-2.25zM13.5 6a2.25 2.25 0 012.25-2.25H18A2.25 2.25 0 0120.25 6v2.25A2.25 2.25 0 0118 10.5h-2.25a2.25 2.25 0 01-2.25-2.25V6zM13.5 15.75a2.25 2.25 0 012.25-2.25H18a2.25 2.25 0 012.25 2.25V18A2.25 2.25 0 0118 20.25h-2.25A2.25 2.25 0 0113.5 18v-2.25z" />
                                            </svg>
                                            <span
                                                class="{{ request()->routeIs('dashboard') || request()->routeIs('admin.dashboard') || request()->routeIs('technician.dashboard') ? 'text-white' : 'text-gray-900 dark:text-white group-hover:text-white' }}"
                                                style="font-size: 15px; font-weight: 500; {{ !(request()->routeIs('dashboard') || request()->routeIs('admin.dashboard') || request()->routeIs('technician.dashboard')) ? 'color: #111827 !important;' : '' }}">Dashboard</span>
                                        </a>
                                    </li>
                                    {{-- <li>
                                        <a href="{{ route('calendar') }}"
                                            class="group flex gap-x-3 rounded-md p-2 text-sm leading-6 font-semibold {{ request()->routeIs('calendar') ? 'bg-gray-800 text-white' : 'text-gray-400 hover:text-white hover:bg-gray-800' }}">
                                    ...
                                    {{-- <li>
                                        <a href="{{ route('work-orders.index') }}"
                                            class="group flex gap-x-3 rounded-md p-2 text-sm leading-6 font-semibold {{ request()->routeIs('work-orders.*') ? 'bg-gray-800 text-white' : 'text-gray-400 hover:text-white hover:bg-gray-800' }}">
                                            <svg class="h-6 w-6 shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                                                stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round"
                                    ...
                                    @if(auth()->user()->hasRole('super-admin'))
                                        <li>
                                            <a href="{{ route('admin.clients.index') ?? route('clients.index') ?? '#' }}"
                                                class="group flex items-center gap-x-6 rounded-md px-3 py-3 text-sm leading-5 font-medium {{ request()->routeIs('clients.*') || request()->routeIs('admin.clients.*') ? 'bg-green-500 text-white' : 'text-gray-900 dark:text-white hover:bg-gray-800 hover:text-white dark:hover:bg-gray-800' }}"
                                                style="{{ !(request()->routeIs('clients.*') || request()->routeIs('admin.clients.*')) ? 'color: #111827 !important;' : '' }}">
                                                <svg class="h-5 w-5 shrink-0 {{ request()->routeIs('clients.*') || request()->routeIs('admin.clients.*') ? 'text-white' : 'text-gray-900 dark:text-gray-400 group-hover:text-white' }}"
                                                    style="margin-right: 20px !important; {{ !(request()->routeIs('clients.*') || request()->routeIs('admin.clients.*')) ? 'color: #111827 !important;' : '' }}"
                                                    fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                                        d="M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z" />
                                                </svg>
                                                <span
                                                    class="{{ request()->routeIs('clients.*') || request()->routeIs('admin.clients.*') ? 'text-white' : 'text-gray-900 dark:text-white group-hover:text-white' }}"
                                                    style="font-size: 15px; font-weight: 500; {{ !(request()->routeIs('clients.*') || request()->routeIs('admin.clients.*')) ? 'color: #111827 !important;' : '' }}">Clientes</span>
                                            </a>
                                        </li>
                                    @endif
                                    {{-- <li>
                                        <a href="{{ route('sites.index') }}"
                                            class="group flex gap-x-3 rounded-md p-2 text-sm leading-6 font-semibold {{ request()->routeIs('sites.*') ? 'bg-gray-800 text-white' : 'text-gray-400 hover:text-white hover:bg-gray-800' }}">
                                            <svg class="h-6 w-6 shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                                                stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round"
                                    ...
                                    <li>
                                        <a href="{{ auth()->user()->hasRole('super-admin') ? (route('admin.services.index') ?? route('services.index') ?? '#') : route('technician.services') }}"
                                            class="group flex items-center gap-x-6 rounded-md px-3 py-3 text-sm leading-5 font-medium {{ request()->routeIs('services.*') || request()->routeIs('admin.services.*') || request()->routeIs('technician.services') ? 'bg-green-500 text-white' : 'text-gray-900 dark:text-white hover:bg-gray-800 hover:text-white dark:hover:bg-gray-800' }}"
                                            style="{{ !(request()->routeIs('services.*') || request()->routeIs('admin.services.*') || request()->routeIs('technician.services')) ? 'color: #111827 !important;' : '' }}">
                                            <svg class="h-5 w-5 shrink-0 {{ request()->routeIs('services.*') || request()->routeIs('admin.services.*') || request()->routeIs('technician.services') ? 'text-white' : 'text-gray-900 dark:text-gray-400 group-hover:text-white' }}"
                                                style="margin-right: 20px !important; {{ !(request()->routeIs('services.*') || request()->routeIs('admin.services.*') || request()->routeIs('technician.services')) ? 'color: #111827 !important;' : '' }}"
                                                fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round"
                                                    d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5a2.25 2.25 0 002.25-2.25m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5a2.25 2.25 0 012.25 2.25v7.5" />
                                            </svg>
                                            <span
                                                class="{{ request()->routeIs('services.*') || request()->routeIs('admin.services.*') || request()->routeIs('technician.services') ? 'text-white' : 'text-gray-900 dark:text-white group-hover:text-white' }}"
                                                style="font-size: 15px; font-weight: 500; {{ !(request()->routeIs('services.*') || request()->routeIs('admin.services.*') || request()->routeIs('technician.services')) ? 'color: #111827 !important;' : '' }}">{{ auth()->user()->hasRole('super-admin') ? 'Servicios' : 'Mis Servicios' }}</span>
                                        </a>
                                    </li>
                                    @if(auth()->user()->hasRole('technician'))
                                        <li>
                                            <a href="{{ route('technician.profile') }}"
                                                class="group flex items-center gap-x-6 rounded-md px-3 py-3 text-sm leading-5 font-medium {{ request()->routeIs('technician.profile') ? 'bg-green-500 text-white' : 'text-gray-900 dark:text-white hover:bg-gray-800 hover:text-white dark:hover:bg-gray-800' }}"
                                                style="{{ !request()->routeIs('technician.profile') ? 'color: #111827 !important;' : '' }}">
                                                <svg class="h-5 w-5 shrink-0 {{ request()->routeIs('technician.profile') ? 'text-white' : 'text-gray-900 dark:text-gray-400 group-hover:text-white' }}"
                                                    style="margin-right: 20px !important; {{ !request()->routeIs('technician.profile') ? 'color: #111827 !important;' : '' }}"
                                                    fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                                        d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z" />
                                                </svg>
                                                <span
                                                    class="{{ request()->routeIs('technician.profile') ? 'text-white' : 'text-gray-900 dark:text-white group-hover:text-white' }}"
                                                    style="font-size: 15px; font-weight: 500; {{ !request()->routeIs('technician.profile') ? 'color: #111827 !important;' : '' }}">Mi
                                                    Perfil</span>
                                            </a>
                                        </li>
                                    @endif
                                    @if(auth()->user()->hasRole('super-admin'))
                                        <li>
                                            <a href="{{ route('admin.reports.index') ?? '#' }}"
                                                class="group flex items-center gap-x-6 rounded-md px-3 py-3 text-sm leading-5 font-medium {{ request()->routeIs('admin.reports.*') || request()->routeIs('reports.*') ? 'bg-green-500 text-white' : 'text-gray-900 dark:text-white hover:bg-gray-800 hover:text-white dark:hover:bg-gray-800' }}"
                                                style="{{ !(request()->routeIs('admin.reports.*') || request()->routeIs('reports.*')) ? 'color: #111827 !important;' : '' }}">
                                                <svg class="h-5 w-5 shrink-0 {{ request()->routeIs('admin.reports.*') || request()->routeIs('reports.*') ? 'text-white' : 'text-gray-900 dark:text-gray-400 group-hover:text-white' }}"
                                                    style="margin-right: 20px !important; {{ !(request()->routeIs('admin.reports.*') || request()->routeIs('reports.*')) ? 'color: #111827 !important;' : '' }}"
                                                    fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                                        d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" />
                                                </svg>
                                                <span
                                                    class="{{ request()->routeIs('admin.reports.*') || request()->routeIs('reports.*') ? 'text-white' : 'text-gray-900 dark:text-white group-hover:text-white' }}"
                                                    style="font-size: 15px; font-weight: 500; {{ !(request()->routeIs('admin.reports.*') || request()->routeIs('reports.*')) ? 'color: #111827 !important;' : '' }}">Reportes</span>
                                            </a>
                                        </li>
                                        <li>
                                            <a href="{{ route('admin.users.index', ['role' => 'technician']) }}"
                                                class="group flex items-center gap-x-6 rounded-md px-3 py-3 text-sm leading-5 font-medium {{ request()->routeIs('admin.users.index') && request()->get('role') === 'technician' ? 'bg-green-500 text-white' : 'text-gray-900 dark:text-white hover:bg-gray-800 hover:text-white dark:hover:bg-gray-800' }}"
                                                style="{{ !(request()->routeIs('admin.users.index') && request()->get('role') === 'technician') ? 'color: #111827 !important;' : '' }}">
                                                <svg class="h-5 w-5 shrink-0 {{ request()->routeIs('admin.users.index') && request()->get('role') === 'technician' ? 'text-white' : 'text-gray-900 dark:text-gray-400 group-hover:text-white' }}"
                                                    style="margin-right: 20px !important; {{ !(request()->routeIs('admin.users.index') && request()->get('role') === 'technician') ? 'color: #111827 !important;' : '' }}"
                                                    fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                                        d="M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z" />
                                                </svg>
                                                <span
                                                    class="{{ request()->routeIs('admin.users.index') && request()->get('role') === 'technician' ? 'text-white' : 'text-gray-900 dark:text-white group-hover:text-white' }}"
                                                    style="font-size: 15px; font-weight: 500; {{ !(request()->routeIs('admin.users.index') && request()->get('role') === 'technician') ? 'color: #111827 !important;' : '' }}">Técnicos</span>
                                            </a>
                                        </li>
                                        <li>
                                            <a href="{{ route('admin.pests') ?? '#' }}"
                                                class="group flex items-center gap-x-6 rounded-md px-3 py-3 text-sm leading-5 font-medium {{ request()->routeIs('admin.pests') ? 'bg-green-500 text-white' : 'text-gray-900 dark:text-white hover:bg-gray-800 hover:text-white dark:hover:bg-gray-800' }}"
                                                style="{{ !request()->routeIs('admin.pests') ? 'color: #111827 !important;' : '' }}">
                                                <svg class="h-5 w-5 shrink-0 {{ request()->routeIs('admin.pests') ? 'text-white' : 'text-gray-900 dark:text-gray-400 group-hover:text-white' }}"
                                                    style="margin-right: 20px !important; {{ !request()->routeIs('admin.pests') ? 'color: #111827 !important;' : '' }}"
                                                    fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                                ...
                                                <span
                                                    class="{{ request()->routeIs('admin.pests') ? 'text-white' : 'text-gray-900 dark:text-white group-hover:text-white' }}"
                                                    style="font-size: 15px; font-weight: 500; {{ !request()->routeIs('admin.pests') ? 'color: #111827 !important;' : '' }}">Plagas</span>
                                            </a>
                                        </li>
                                        <li>
                                            <a href="{{ route('admin.products.index') ?? '#' }}"
                                                class="group flex items-center gap-x-6 rounded-md px-3 py-3 text-sm leading-5 font-medium {{ request()->routeIs('admin.products.*') ? 'bg-green-500 text-white' : 'text-gray-900 dark:text-white hover:bg-gray-800 hover:text-white dark:hover:bg-gray-800' }}"
                                                style="{{ !request()->routeIs('admin.products.*') ? 'color: #111827 !important;' : '' }}">
                                                <svg class="h-5 w-5 shrink-0 {{ request()->routeIs('admin.products.*') ? 'text-white' : 'text-gray-900 dark:text-gray-400 group-hover:text-white' }}"
                                                    style="margin-right: 20px !important; {{ !request()->routeIs('admin.products.*') ? 'color: #111827 !important;' : '' }}"
                                                    fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                                        d="M20.25 7.5l-.625 10.632a2.25 2.25 0 01-2.247 2.118H6.622a2.25 2.25 0 01-2.247-2.118L3.75 7.5M10 11.25h4M3.375 7.5h17.25c.621 0 1.125-.504 1.125-1.125v-1.5c0-.621-.504-1.125-1.125-1.125H3.375c-.621 0-1.125.504-1.125 1.125v1.5c0 .621.504 1.125 1.125 1.125z" />
                                                </svg>
                                                <span
                                                    class="{{ request()->routeIs('admin.products.*') ? 'text-white' : 'text-gray-900 dark:text-white group-hover:text-white' }}"
                                                    style="font-size: 15px; font-weight: 500; {{ !request()->routeIs('admin.products.*') ? 'color: #111827 !important;' : '' }}">Productos</span>
                                            </a>
                                        </li>
                                    @endif
                                    {{-- <li>
                                        <a href="{{ route('materials.index') }}"
                                            class="group flex gap-x-3 rounded-md p-2 text-sm leading-6 font-semibold {{ request()->routeIs('materials.*') ? 'bg-gray-800 text-white' : 'text-gray-400 hover:text-white hover:bg-gray-800' }}">
                                            <svg class="h-6 w-6 shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                                                stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round"
                                                    d="M20.25 7.5l-.625 10.632a2.25 2.25 0 01-2.247 2.118H6.622a2.25 2.25 0 01-2.247-2.118L3.75 7.5M10 11.25h4M3.375 7.5h17.25c.621 0 1.125-.504 1.125-1.125v-1.5c0-.621-.504-1.125-1.125-1.125H3.375c-.621 0-1.125.504-1.125 1.125v1.5c0 .621.504 1.125 1.125 1.125z" />
                                    ...
                                    {{-- <li>
                                        <a href="{{ route('reports.index') }}"
                                            class="group flex gap-x-3 rounded-md p-2 text-sm leading-6 font-semibold {{ request()->routeIs('reports.*') ? 'bg-gray-800 text-white' : 'text-gray-400 hover:text-white hover:bg-gray-800' }}">
                                            <svg class="h-6 w-6 shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                                                stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round"
                                ...
                                <ul role="list" class="-mx-2 space-y-1.5">
                                    <li>
                                        <a href="{{ route('admin.users.index') }}"
                                            class="group flex items-center gap-x-6 rounded-md px-3 py-3 text-sm leading-5 font-medium {{ request()->routeIs('admin.users.*') ? 'bg-green-500 text-white' : 'text-gray-900 dark:text-white hover:bg-gray-800 hover:text-white dark:hover:bg-gray-800' }}">
                                            <svg class="h-5 w-5 shrink-0 {{ request()->routeIs('admin.users.*') ? 'text-white' : 'text-gray-900 dark:text-gray-400 group-hover:text-white' }}"
                                                style="margin-right: 20px !important; {{ !request()->routeIs('admin.users.*') ? 'color: #111827 !important;' : '' }}"
                                                fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round"
                                                    d="M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z" />
                                            </svg>
                                            <span
                                                class="{{ request()->routeIs('admin.users.*') ? 'text-white' : 'text-gray-900 dark:text-white group-hover:text-white' }}"
                                                style="font-size: 15px; font-weight: 500; {{ !request()->routeIs('admin.users.*') ? 'color: #111827 !important;' : '' }}">Usuarios</span>
                                        </a>
                                    </li>
                                    <li>
                                        <a href="{{ route('admin.notification-center') ?? '#' }}"
                                            class="group flex items-center gap-x-6 rounded-md px-3 py-3 text-sm leading-5 font-medium {{ request()->routeIs('admin.notification-center') ? 'bg-green-500 text-white' : 'text-gray-900 dark:text-white hover:bg-gray-800 hover:text-white dark:hover:bg-gray-800' }}"
                                            style="{{ !request()->routeIs('admin.notification-center') ? 'color: #111827 !important;' : '' }}">
                                            <svg class="h-5 w-5 shrink-0 {{ request()->routeIs('admin.notification-center') ? 'text-white' : 'text-gray-900 dark:text-gray-400 group-hover:text-white' }}"
                                                style="margin-right: 20px !important; {{ !request()->routeIs('admin.notification-center') ? 'color: #111827 !important;' : '' }}"
                                                fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round"
                                                    d="M14.857 17.082a23.848 23.848 0 005.454-1.31A8.967 8.967 0 0118 9.75v-.7V9A6 6 0 006 9v.75a8.967 8.967 0 01-2.312 6.022c1.733.64 3.56 1.085 5.455 1.31m5.714 0a24.255 24.255 0 01-5.714 0m5.714 0a3 3 0 11-5.714 0" />
                                            </svg>
                                            <span
                                                class="{{ request()->routeIs('admin.notification-center') ? 'text-white' : 'text-gray-900 dark:text-white group-hover:text-white' }}"
                                                style="font-size: 15px; font-weight: 500; {{ !request()->routeIs('admin.notification-center') ? 'color: #111827 !important;' : '' }}">Notificaciones</span>
                                        </a>
                                    </li>
                                    <li>
                                        <a href="{{ Route::has('admin.settings') ? route('admin.settings') : '#' }}"
                                            class="group flex items-center gap-x-6 rounded-md px-3 py-3 text-sm leading-5 font-medium {{ request()->routeIs('admin.settings.*') || request()->routeIs('admin.settings') ? 'bg-green-500 text-white' : 'text-gray-900 dark:text-white hover:bg-gray-800 hover:text-white dark:hover:bg-gray-800' }}"
                                            style="{{ !(request()->routeIs('admin.settings.*') || request()->routeIs('admin.settings')) ? 'color: #111827 !important;' : '' }}">
                                            <svg class="h-5 w-5 shrink-0 {{ (request()->routeIs('admin.settings.*') || request()->routeIs('admin.settings')) ? 'text-white' : 'text-gray-900 dark:text-gray-400 group-hover:text-white' }}"
                                                style="margin-right: 20px !important; {{ !(request()->routeIs('admin.settings.*') || request()->routeIs('admin.settings')) ? 'color: #111827 !important;' : '' }}"
                                                fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round"
                                            ...
                                            <span
                                                class="{{ (request()->routeIs('admin.settings.*') || request()->routeIs('admin.settings')) ? 'text-white' : 'text-gray-900 dark:text-white group-hover:text-white' }}"
                                                style="font-size: 15px; font-weight: 500; {{ !(request()->routeIs('admin.settings.*') || request()->routeIs('admin.settings')) ? 'color: #111827 !important;' : '' }}">Configuraciones</span>
                                        </a>
                                    </li>
                                    {{-- <li>
        ...
        <div class="flex-1 flex flex-col min-w-0 overflow-x-hidden">
            ...
            <header class="md:hidden fixed top-0 left-0 right-0 bg-white border-b border-gray-200"
                style="width: 100%; z-index: 9999;">
                <div class="w-full px-4 py-3 flex items-center justify-between gap-4">
                    <!-- Left: Mobile Menu Button -->
                    <button id="header-mobile-menu-button"
                        class="flex-shrink-0 p-2 rounded-lg bg-white border border-gray-300 shadow-md hover:bg-gray-50 transition-colors"
                        style="z-index: 50; background-color: rgb(255, 255, 255);">
                        <svg id="header-menu-icon" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="2"
                            stroke="currentColor" class="text-gray-900 dark:text-white">
                            <path stroke-linecap="round" stroke-linejoin="round"
                                d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5"></path>
                        </svg>
                        <svg id="header-close-icon" class="h-5 w-5 hidden" fill="none" viewBox="0 0 24 24"
                            stroke-width="2" stroke="currentColor" class="text-gray-900 dark:text-white">
                    ...
                    <div class="flex items-center gap-3 flex-shrink-0">
                        ...
                        <div class="relative" id="header-notification-dropdown">
                            <button type="button" id="header-notification-button"
                                class="relative flex items-center justify-center w-10 h-10 rounded-full hover:bg-gray-100 transition-colors">
                                <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                                    stroke="currentColor" class="text-gray-600 dark:text-white">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                                        d="M14.857 17.082a23.848 23.848 0 005.454-1.31A8.967 8.967 0 0118 9.75v-.7V9A6 6 0 006 9v.75a8.967 8.967 0 01-2.312 6.022c1.733.64 3.56 1.085 5.455 1.31m5.714 0a24.255 24.255 0 01-5.714 0m5.714 0a3 3 0 11-5.714 0" />
                                </svg>
                                @php
                                ...
                                @if($unreadCount > 0)
                                    <span
                                        class="absolute text-white text-xs rounded-full flex items-center justify-center font-semibold">
                            ...
                            <div id="header-notification-menu"
                                ...
                                <div class="p-3 border-b border-gray-200 flex justify-between items-center">
                                    ...
                                    @if(Route::has('admin.notification-center'))
                                        <a href="{{ auth()->user()->hasRole('super-admin') ? route('admin.notification-center') : route('technician.notifications.index') }}"
                                            class="text-sm text-green-600 hover:text-green-700">Ver todas</a>
                                    @endif
                                </div>
                                <div class="p-2">
                                    @php
                                    ...
                                    @if($recentNotifications->count() > 0)
                                        @foreach($recentNotifications as $notification)
                                            ...
                                            @endphp
                                            <div
                                                class="p-3 hover:bg-gray-50 rounded-lg cursor-pointer {{ !$isRead ? 'bg-green-50' : '' }}">
                        ...
                        <div class="relative" id="header-user-dropdown">
                            <button type="button" id="header-user-button"
                            ...
                            <div id="header-user-menu"
                                ...
                                <div class="p-3 border-b border-gray-200">
                                    <div class="flex items-center gap-3">
                                        <div
                                            class="h-10 w-10 rounded-full bg-green-500 flex items-center justify-center flex-shrink-0">
                                            <span class="text-sm font-medium text-white">
                                                {{ auth()->check() ? strtoupper(substr(auth()->user()->name, 0, 1)) : 'U' }}
                                            </span>
                                        </div>
                                ...
                                <div class="p-2">
                                    ...
                                    <form method="POST" action="{{ route('logout') }}">
                                        @csrf
                                        <button type="submit"
                                            class="w-full flex items-center gap-3 px-3 py-2 text-sm text-gray-700 hover:bg-gray-100 rounded-lg">
                                            <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                                                stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round"
