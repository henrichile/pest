            ...
            <div class="signature-display">
                @if(auth()->user()->signature)
                    ...
                    <div class="signature-preview">
                        <img src="{{ Storage::url(auth()->user()->signature) }}" alt="Firma del Técnico"
    ...
    <div class="form-section">
        ...
        <div class="form-grid">
            <div>
                ...
                <input type="text" name="signer_name" id="signer_name" class="form-input"
                    value="{{ old('signer_name', $monitoreoFirma['signer_name'] ?? $service->client->name ?? '') }}"
                    required>
            </div>
            <div>
                <label>Cargo / Relación</label>
                <select name="signer_position" id="signer_position"
                    class="form-select dark:text-white dark:bg-gray-700 dark:border-gray-600">
                    <option value="">Seleccionar</option>
                    <option value="encargado" {{ old('signer_position', $monitoreoFirma['signer_position'] ?? $service->checklist_data['signer_position'] ?? '') === 'encargado' ? 'selected' : '' }}>Encargado
                    </option>
                    <option value="gerente" {{ old('signer_position', $monitoreoFirma['signer_position'] ?? $service->checklist_data['signer_position'] ?? '') === 'gerente' ? 'selected' : '' }}>Gerente
                    </option>
                    <option value="propietario" {{ old('signer_position', $monitoreoFirma['signer_position'] ?? $service->checklist_data['signer_position'] ?? '') === 'propietario' ? 'selected' : '' }}>
                        Propietario</option>
                    <option value="representante" {{ old('signer_position', $monitoreoFirma['signer_position'] ?? $service->checklist_data['signer_position'] ?? '') === 'representante' ? 'selected' : '' }}>
