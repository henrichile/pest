...
@section('content')
    <div class="space-y-4 sm:space-y-6 pt-12 md:pt-0">
        ...
        <div class="md:flex md:items-center md:justify-between mb-6">
            <div class="min-w-0 flex-1">
                <h2 class="text-3xl font-bold leading-7 text-gray-900 sm:truncate sm:text-3xl sm:tracking-tight text-gray-900 dark:text-white"
                    class="font-bold">
            ...
            <div class="mt-4 md:mt-0 md:ml-4 flex flex-col sm:flex-row gap-2">
                ...
                <button type="button" onclick="openSendModal()"
                    class="inline-flex items-center px-4 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 transition-colors">
                    <svg class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
        ...
        @if(session('success'))
            <div class="bg-green-50 border border-green-200 text-green-800 px-4 py-3 rounded-lg mb-4"
                style="border: 1px solid #22c55e !important;">
                {{ session('success') }}
            </div>
        @endif
        ...
        @if($errors->any())
            <div class="bg-red-50 border border-red-200 text-red-800 px-4 py-3 rounded-lg mb-4"
                style="border: 1px solid #ef4444 !important;">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
            <div
                class="bg-white dark:bg-gray-800 border dark:border-gray-700 rounded-lg p-4 border border-gray-200 dark:border-gray-700">
        ...
        <!-- Filters -->
        <div
            class="bg-white dark:bg-gray-800 border dark:border-gray-700 rounded-lg p-4 mb-6 border border-gray-200 dark:border-gray-700">
            <form method="GET" action="{{ route('admin.notification-center') }}"
                ...
                <div>
                    ...
                    <select id="type" name="type"
                        class="block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-sm text-gray-900 dark:text-white dark:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent dark:text-white dark:bg-gray-700 dark:border-gray-600"
                        style="border: 1px solid #e5e7eb !important;">
                ...
                <div>
                    <label for="read_status"
                        class="block text-sm font-medium mb-2 text-gray-700 dark:text-white">Estado</label>
                ...
                <div>
                    <label for="start_date"
                        class="block text-sm font-medium mb-2 text-gray-700 dark:text-white">Desde</label>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200" style="table-layout: fixed; width: 100%;">
                    ...
                    <thead class="bg-gray-50">
                        <tr>
                            <th
                                class="px-3 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-600 dark:text-white">
                                Usuario</th>
                            <th
                                class="px-2 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-600 dark:text-white">
                                Tipo</th>
                            <th
                                class="px-3 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-600 dark:text-white">
                                Título</th>
                            ...
                            <th
                                ...
                                Fecha</th>
                            <th
                                class="px-3 py-3 text-right text-xs font-medium uppercase tracking-wider text-gray-600 dark:text-white">
                                Acciones</th>
                        </tr>
                    </thead>
                                    ...
                                    <div class="flex flex-col gap-1 items-end">
                                        @if(!$notification->read_at)
                                            <form action="{{ route('admin.notifications.mark-read', $notification->id) }}"
                                                ...
                                                @method('PATCH')
                                                <button type="submit"
                                                    class="text-green-600 hover:text-green-900 text-xs whitespace-nowrap">Marcar
                                                    como leída</button>
                                            </form>
                                        @endif
                                        <form action="{{ route('admin.notifications.destroy', $notification->id) }}"
                                            ...
                                            @method('DELETE')
                                            <button type="submit"
                                                class="text-red-600 hover:text-red-900 text-xs whitespace-nowrap">Eliminar</button>
                                        </form>
                                    </div>
                                </td>
                            </tr>
    ...
    <div id="send-modal" class="fixed inset-0 z-50 hidden overflow-y-auto" aria-labelledby="modal-title" role="dialog"
        aria-modal="true">
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            <div class="fixed inset-0 transition-opacity bg-gray-500 bg-opacity-75" aria-hidden="true" id="modal-overlay"
                onclick="closeSendModal()"></div>

            <div
                class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full border border-gray-200 dark:border-gray-700">
                <form action="{{ route('admin.notifications.send') }}" method="POST">
                    ...
                    <div class="bg-white px-6 pt-5 pb-4 sm:p-6 sm:pb-4">
                        ...
                        <div class="space-y-4">
                            <div>
                                <label for="user_ids"
                                    class="block text-sm font-medium mb-2 text-gray-700 dark:text-white">Usuarios *</label>
                                <select id="user_ids" name="user_ids[]" multiple required
                                    class="block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-sm text-gray-900 dark:text-white dark:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent dark:text-white dark:bg-gray-700 dark:border-gray-600"
                                    style="border: 1px solid #e5e7eb !important; min-height: 100px;">
                                    @foreach($users as $user)
                                        <option value="{{ $user->id }}">{{ $user->name }} ({{ $user->email }})</option>
                                    @endforeach
                                </select>
                                <p class="mt-1 text-xs text-gray-600 dark:text-white">Mantén presionado Ctrl (Cmd en Mac)
                                    para seleccionar múltiples usuarios</p>
                            </div>

                            <div>
                                <label for="title"
                                    class="block text-sm font-medium mb-2 text-gray-700 dark:text-white">Título *</label>
                            ...
                            <div>
                                <label for="message"
                                    class="block text-sm font-medium mb-2 text-gray-700 dark:text-white">Mensaje *</label>
                            ...
                            <div>
                                <label for="type" class="block text-sm font-medium mb-2 text-gray-700 dark:text-white">Tipo
                                    *</label>
                            ...
                            <div>
                                <label for="url" class="block text-sm font-medium mb-2 text-gray-700 dark:text-white">URL
                                    (opcional)</label>
                    ...
                    <div class="bg-gray-50 px-6 py-4 sm:flex sm:flex-row-reverse">
                        <button type="submit"
                            class="w-full sm:w-auto sm:ml-3 px-4 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 transition-colors">
                            Enviar Notificación
                        </button>
                        <button type="button" onclick="closeSendModal()"
                            class="mt-3 sm:mt-0 w-full sm:w-auto px-4 py-2 bg-gray-200 text-gray-800 rounded-lg hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition-colors">
                            Cancelar
                        </button>
                    </div>
                </form>
