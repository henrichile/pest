            ...
            <div class="flex justify-between items-start mb-4">
                <div>
                    <h2 class="text-2xl font-bold text-gray-900 dark:text-white">
                        {{ $service->client->name ?? "Cliente no encontrado" }}</h2>
            ...
            <div class="bg-white rounded-lg shadow-lg p-6">
                ...
                <div class="space-y-3">
                    ...
                    <div>
                        ...
                        <p class="text-gray-900 dark:text-white">{{ $service->client->business_type ?? "No especificado" }}
                        </p>
                    </div>
                </div>
            </div>
        ...
        @if($service->checklist_data && ($service->status === 'finalizado' || $service->status === 'completado'))
            ...
            @if(!$isMonitoreoCebaderas)
                ...
                @if(isset($checklistData['products']))
                    <div class="bg-white rounded-lg shadow-lg p-6">
                        <h3 class="text-lg font-semibold text-gray-900 mb-4 dark:text-white">Producto Aplicado </h3>
                ...
                @if(isset($checklistData['observations']) && count($checklistData['observations']) > 0)
                    <div class="bg-white rounded-lg shadow-lg p-6">
                        <h3 class="text-lg font-semibold text-gray-900 mb-4 dark:text-white">Observaciones
                            ({{ count($checklistData['observations']) }})</h3>
                        <div class="space-y-3">
                                ...
                                <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                                    <div class="flex items-start justify-between">
                                        ...
                                        @if(isset($observation['created_at']))
                                            <span
                                                class="text-xs text-gray-500">{{ \Carbon\Carbon::parse($observation['created_at'])->format('d/m/Y H:i') }}</span>
                                        @endif
                                    </div>
                ...
                @if(isset($checklistData['sites']['treated_sites']))
                    <div class="bg-white rounded-lg shadow-lg p-6">
                        ...
                        <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                            <p class="text-gray-700 dark:text-white whitespace-pre-wrap dark:text-gray-400">
                                {{ $checklistData['sites']['treated_sites'] }}</p>
                        </div>
                    </div>
                @endif
                    ...
                    <div class="bg-white rounded-lg shadow-lg p-6">
                        ...
                        @if(isset($checklistData['description']['service_description']) && !empty($checklistData['description']['service_description']))
                            <div class="mb-4">
                                ...
                                <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                                    <p class="text-gray-700 whitespace-pre-wrap dark:text-gray-400">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    @if($techSignature)
                        ...
                        <p class="text-sm text-gray-600 mt-1 dark:text-gray-400">{{ $service->assignedUser->name ?? 'Técnico' }}
                        </p>
                    @else
                    ...
                    @if($clientSignature)
                        ...
                        <p class="text-sm text-gray-600 mt-1">
                            ...
                            @if(isset($service->checklist_data['monitoreo_firma']['signer_position']))
                                <span
                                    class="text-xs text-gray-500">({{ ucfirst($service->checklist_data['monitoreo_firma']['signer_position']) }})</span>
                            @endif
                        </p>
                    @else
                        <p class="text-gray-500 italic dark:text-gray-400">No registrada</p>
                ...
                <div class="flex items-center justify-between flex-wrap gap-4">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-green-700 dark:text-green-400">Puedes descargar el informe completo en PDF.
                            </p>
                        </div>
                    </div>
